...
function Lyrics() {
    ...
    const loadLyricsForTrack = async () => {
      if (currentTrack) {
        try {
          ...
          const savedLyrics = await window.electron.getLyrics(
  ...
  const handleLrcUpload = async (
    ...
    try {
      if (file && currentTrack) {
        ...
        const existingLyrics = await window.electron.getLyrics(
          currentTrack.song_id,
        );
  ...
  return (
    <div
      className={`lyrics-view ${isLyricsVisible ? "block" : "hidden"} w-xs bg-[#00000038] flex flex-col`}
    >
      <div className="p-4 gap-4 flex justify-between items-center">
        ...
        <div className="flex gap-2">
          ...
          />
          <Dropdown>
            <DropdownItem onClick={openSingInfo} disabled={!currentTrack}>
              Song Info
            </DropdownItem>
            <DropdownItem
            ...
            >
              {currentTrack?.lyrics ? "Change Lyrics File" : "Add Lyrics File"}
            </DropdownItem>
            <DropdownSeparator />
            <DropdownItem onClick={openSettingsModal}>Settings</DropdownItem>
            <DropdownItem onClick={handleLogoutClick}>About</DropdownItem>
          </Dropdown>
        </div>
      </div>
      <div className="text-white h-full overflow-y-auto">
        {lyrics.length > 0 ? (
          ...
          <div className="flex flex-col items-center justify-center h-full">
            <p className="opacity-70 text-xl font-semibold">
              No lyrics available
            </p>
            <p className="opacity-40 text-sm">
              Try uploading a lyrics file (.lrc)
            </p>
          </div>
        )}
      </div>
